# FRD Repeatability Development - 2026-09-03 To 2026-09-05

Lifecycle: **HISTORY — AUDIT ONLY; DO NOT USE AS A CURRENT PROCEDURE**

This record preserves the development of the UMC202HD full-dual method, its
suspension after desktop source-stream dropouts, and the later UMC22 checkout
and repeatability narrative. It deliberately retains obsolete settings and
superseded instructions as dated evidence. They are not authorisation to power
equipment or generate a signal.

Use [`../FRD_MEASUREMENT_METHOD.md`](../FRD_MEASUREMENT_METHOD.md) for the
interface-neutral method, use
[`../procedures/UMC22_INSTALLED_WOOFER_FRD.md`](../procedures/UMC22_INSTALLED_WOOFER_FRD.md)
for current UMC22 work, and use
[`../procedures/UMC202HD_FRD_SUSPENDED.md`](../procedures/UMC202HD_FRD_SUSPENDED.md)
for the UMC202HD suspension/resumption boundary.
...
[the consolidated fixture qualification](../qualification/REFERENCE_FIXTURE_QUALIFICATION_2026-09-01_TO_03.md)
...
[the current UMC22 procedure](../procedures/UMC22_INSTALLED_WOOFER_FRD.md);
...
### 10.1 First Reusable UMC22 Installed-Woofer Capture

**USER-REPORTED START STATE - 2026-09-06:** the user reports that the entire
system remains in a state identical to the passed qualification and that no
anomaly is observed. This is accepted as a current-state report covering the
qualified physical wiring, powered controls, `1000 mm` on-axis geometry,
enclosure/room state, and stopped-signal condition. It is not a new electrical
or acoustic measurement.

Before any new signal, leave every physical control, connection, and position
unchanged and perform this no-excitation audit:

1. Confirm Generator, Check Levels, and Measure are all stopped and there is no
   other application audio.
2. Confirm REW V5.40 beta 133, Java `Stereo only`, exact input and output
   `EXCL: Behringer UMC22 (USB Audio CODEC)`, `48 kHz`, input-volume control
   at `1.00`, and no separately loaded soundcard-calibration file.
3. Confirm measurement output L, timing-reference output L, measurement input
   `MICROPHONE (Master Volume) L`, and reference input
   `MICROPHONE (Master Volume) R`.
4. Confirm `Use loopback as cal and timing reference`, `Make calibration data
   from loopback response`, `Merge loopback response into IR` off, and timing
   offset `0.0000 ms`.
5. In Measure's Calibration data display, confirm
   `ECM8000_calibration_data.csv` for the exact active measurement input L and
   no microphone calibration on reference input R.
6. Confirm both UMC22 clip indicators remain off and the idle system remains
   silent and free of any new abnormality.
7. Prepare, but do not start, a single SPL sweep named
   `SB17 installed 0deg 1000mm UMC22 2026-09-06 capture 1`, with
   `20-20000 Hz`, `-20 dBFS`, `256k`, one repetition, and clipping abort
   enabled.

Report the displayed settings and any warning or changed field before running
the sweep. Only after this audit is reviewed may one capture be authorised.
That capture must be followed by review of both input headrooms, stored
microphone/soundcard/routing/timing metadata, warnings or anomalies, the direct
impulse and ETC, and a proposed REW-native window. Do not repeat, rotate the
cabinet, export FRD, or adopt the derived qualification window before that
review.

Primary references:

- Complete reference-fixture authority:
  [`REFERENCE_FIXTURE_SPECIFICATION.md`](../fixtures/REFERENCE_FIXTURE_SPECIFICATION.md)
- Completed fixture AC commissioning:
  [`REFERENCE_FIXTURE_QUALIFICATION_2026-09-01_TO_03.md`](../qualification/REFERENCE_FIXTURE_QUALIFICATION_2026-09-01_TO_03.md)
